    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                /*1) 이번달 수입 */
                const income=result.data.income.filter((v) => v._id === month)
                ...
                //디데이
                const dday=mindate.diff(current,'days')+1
                
                /* 3) 계좌 잔액*/

                setdata({balance:result.data.balance,income:income[0].sum,deposit:0,dday:dday})
            } catch (error) {
                setIsError(true);
            }
            setIsLoading(false);
                                ...
                                <div className="card-body">
                                    <div className="row no-gutters align-items-center">
                                        <div className="col mr-2">
                                            <div className="text-xs font-weight-bold text-success text-uppercase mb-1">
                                                예금</div>
                                            <div className="h5 mb-0 font-weight-bold text-gray-800">D-{data.dday}</div></div>
