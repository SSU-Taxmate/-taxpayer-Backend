...
function DepositAdd({ balance }) {
  ...
  const handleNext = () => {
    if (activeStep == 0) {
      ...
      if (data.amount < data.product.minAmount) {
        return seterrmsg(`최소 가입 금액(${data.product.minAmount}) 이상 입력해주세요`)
  ...
  function getStepContent(stepIndex) {
    switch (stepIndex) {
      case 0:
        return <FirstStep data={data} handleChange={handleChange} seterrmsg={seterrmsg} />;
      case 1:
        return <SecondStep data={data} handleChange={handleChange} balance={balance} />;
      case 2:
  ...
  const onSubmit = (e) => {
    e.preventDefault();
    axios.post(`/api/bank/deposits/${data.product._id}/join`, { studentId: joinedUser.classUser, amount: data.amount })//
  ...
  return (
    <>
      ...
      <Dialog
        open={open}
        aria-labelledby="dialog-depositjoin"
      >
