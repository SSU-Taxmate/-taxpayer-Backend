...
function PenaltyTable(props){
    ...
    function filterRow(status) {
        status !==null ? setData(props.data.filter(data => data.isPayed===status)):setData(props.data)
      }
    ...
    function PaneltyToolbar() {
        return (
         ...
         <GridToolbarContainer className="card-header py-1 d-flex flex-row align-items-center justify-content-between">
            ...
            { user.role === 0?  <ButtonGroup color="primary" variant="text" spacing={1}>
            ...
            <div className="dropdown no-arrow mx-3" >
              ...
              <div className="dropdown-menu dropdown-menu-right shadow animated--fade-in" aria-labelledby="dropdownMenuLink">
                <a className="dropdown-item" role="button" onClick={() => filterRow(null)}>전체</a>
                <a className="dropdown-item" role="button" onClick={() => filterRow(false)}>미납</a>
                <a className="dropdown-item" role="button" onClick={() => filterRow(true)}>납부</a>
              </div>
            </div>
