...
router.get("/", async (req, res) => {
  try {
    ...
    let result = await Promise.all(
      stock.map(async (v, i) => {
        v.prices = await v.prices.filter(
          (price) =>
            price.updateDate <=
...
router.delete("/:id", async (req, res) => {
  ...
  try {
    ...
    session.startTransaction();
    // Stock.ondelete = true
    const notclosed = await StockAccount.countDocuments({
      "holdingStocks.stockId": stockId,
    }).exec({ session });
    ...
    if (notclosed <= 0) {
      ...
      const delclasstock = await ClassStock.deleteOne(
        { stockId: stockId },
        { session: session }
      );
      ...
      //보유중인 사람이 있다면
      //1) 상장폐지 신청
      const ondelete = await Stock.updateOne(
        { _id: stockId },
        {
          $set: {
            ondelete: true,
            ondeleteDay: new Date(+new Date() + 15 * 24 * 60 * 60 * 1000),
          },
        }
      ).exec({ session });
...
router.post("/:id/orders", async (req, res) => {
  ...
  try {
    ...
    const account = await Account.findOne({ studentId: studentId }).exec({
      session,
    });
    ...
    if (orderType === "매수") {
      ...
      if (account.currentBalance >= quantity * currentPrice) {
        ...
        // 3) 은행 처리
        const minus = await Account.updateOne(
          { _id: account._id },
          { $inc: { currentBalance: -history.payAmount } },
          { session }
        );
        ...
        const index = stockaccount.holdingStocks.findIndex(
          (v) => v.stockId == stockId
        );
        //console.log(index)
        if (index > -1) {
          // 4-1) 기존 stock이 있다면,
          const addStock = await StockAccount.updateOne(
            { studentId: studentId },
            {
              $inc: {
      ...
      if (
        index > -1 &&
        quantity <= stockaccount.holdingStocks[index].quantity
      ) {
        //매도수량 확인
        //매도
        const minusStock = await StockAccount.updateOne(
          { studentId: studentId },
          {
            $inc: {
              [`holdingStocks.${index}.quantity`]: -quantity,
        ...
        const pullStock = await StockAccount.updateOne(
          { studentId: studentId },
          {
            $pull: {
        ...
        // 1. 은행 (입금)
        const plus = await Account.updateOne(
          { _id: account._id },
          { $inc: { currentBalance: currentPrice * quantity } },
          { session }
        );
        ...
        const minus = await Account.updateOne(
          { _id: account._id },
          { $inc: { currentBalance: -tax2user } },
          { session }
        );
        //은행 거래 데이터 추가(출금)
        const account2 = await Account.findOne({ _id: account._id }).exec({
          session,
        });
